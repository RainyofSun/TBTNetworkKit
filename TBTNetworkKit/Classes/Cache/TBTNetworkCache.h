...
@class TBTNetworkResponse;

@interface TBTNetworkCache : NSObject

/** 缓存存储模式 (默认不缓存) */
@property (nonatomic, assign) TBTNetworkCacheWriteMode writeMode;

/** 缓存读取模式 (默认不读取) */
@property (nonatomic, assign) TBTNetworkCacheReadMode readMode;
...
@property (nonatomic, copy, nullable) BOOL(^shouldCacheBlock)(TBTNetworkResponse *response);
